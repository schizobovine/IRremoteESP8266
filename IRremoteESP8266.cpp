...
void ICACHE_FLASH_ATTR IRsend::sendRC5(unsigned long long data,
                                       unsigned int nbits,
                                       unsigned int repeat) {
  bool skipSpace = true;
...
// Ref:
//   https://en.wikipedia.org/wiki/Manchester_code
int ICACHE_FLASH_ATTR IRrecv::getRClevel(decode_results *results,
                                         unsigned int *offset,
                                         unsigned int *used,
...
bool ICACHE_FLASH_ATTR IRrecv::decodeRC5(decode_results *results,
                                         uint16_t nbits, bool strict) {
  if (results->rawlen < MIN_RC5_SAMPLES + HEADER)
    return false;
  ...
  unsigned int used = 0;
  unsigned int field_bit;  // a.k.a. second start bit.
  bool is_rc5x = false;
  if (nbits == RC5X_BITS)
    is_rc5x = true;
  ...
  if (is_rc5x)
    actual_bits = 1;  // We've already have our first (field) bit.
  ...
  // Compliance
  if (strict) {
    if (actual_bits != RC5_BITS && actual_bits != RC5X_BITS)
      return false;
